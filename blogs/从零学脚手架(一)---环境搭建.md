...
在当前前端时代，Node.JS已是前端开发必备的开发依赖，往往前端安装的第一个环境就是Node.JS，那么我们也从Node.JS开始
截至到本文编写时间（2020年12月14日），[Node.JS官网](https://nodejs.org/en/)的LTS版本为<font style="color:#f03d3d">14.51.1</font>，最新版本为<font style="color:#f03d3d">15.4.0</font>

<img src=".//images//image-01-01.png" width="400">

cli脚手架并不需要特别新的Node版本，所以也不需要特别激进的使用新版本，在此使用最新版的LTS版本即可。

> :whale2:  Node是一个运行环境，内核使用的是V8引擎，熟悉后端的将它看做JVM或者CLR即可


安装Node除了官方卸载安装其实还有一个更好的选择，那就是使用[NVM](https://github.com/nvm-sh/nvm)工具，NVM是一个Node版本管理工具，这个工具允许同时存在多个Node版本，并且轻易的切换。例如我公司一个老项目依赖Node.11版本，但是现在像有些CSS包依赖Node.12/Node.14这样的版本，所以我本地装了多个Node版本，方便切换。
...
有许多人使用Yarn进行包管理（本人也是使用的yarn）,[Yarn官方](https://yarn.bootcss.com/)说Yarn具有速度快、可靠等优点，不过在yarn出现不久，NPM也紧跟其后，进行优化，现在两种基本没有什么差别了。使用哪个取决于喜好，有兴趣诸君请参考[npm 和 yarn 你选哪个？](https://segmentfault.com/a/1190000021335004)。日常使用yarn与npm也只不过语法单词有些差别<font style="color:#f03d3d">npm install</font>、<font style="color:#f03d3d">yarn add</font> 

> :whale2::whale2:	yarn官方下载安装程序页面404了，诸君可以去[GitHub](https://github.com/yarnpkg/yarn/tags)下载
...
> :whale2::whale2:	模块（Module）引用的外部包存放在node_modules目录下，此目录由于存放所以外部包所以会占用大量磁盘。使用GIT项目管理时要忽略此目录



#### 添加依赖包

在使用层面模块（Module）将包分为生产环境依赖和开发环境依赖，在package.json中分别在<font style="color:#f03d3d">dependencies</font>和<font style="color:#f03d3d">devDependencies</font>属性下，生产环境包比较好理解，就是代码在运行时使用到的依赖，例如Jquery、vue等，都是在实际代码使用到的，至于开发环境依赖的包，则是参与打包器等一系列的包，这种包在后面会详细讲解。
...
> :whale2::whale2:
>
> 1. webpack@5.4包表示添加的是5.4版本的webpack。webpack截至到目前最新版本为5.10.1 但是新版本某些功能具有一定的问题。
>2. jquery包有的会安装在<font style="color:#f03d3d">peerDependencies</font>依赖中，这个依赖提供了一个通用包引用的概念，具体请参考[一文搞懂peerDependencies(https://segmentfault.com/a/1190000022435060)
>    1. 包版本有时具有^、~、*等符号，这些都是匹配最新的版本，具体请参考[NPM依赖包版本号~和^和*的区别](https://blog.csdn.net/p358278505/article/details/78247700)
...
在添加一个包时会在根目录中生成<font style="color:#f03d3d">node_modules目录</font>和<font style="color:#f03d3d">yarn.lock/package-lock.json</font>文件

* **node_modules目录** 当前模块依赖包所存放的位置
* **yarn.lock/package-lock.json** 当前模块依赖包的信息，在<font style="color:#f03d3d">package.json</font>文件中依赖包只有名称和版本号，而详细信息存放在了<font style="color:#f03d3d">yarn.lock/package-lock.json</font>文件中
...
>
> * package.json文件其实就是项目管理的配置文件
> * 以package.json为根目录的目录为一个**模块（Module）**，其模块的外部依赖包和执行命令全部有package文件承载。
