...
在前面说过在打包时分为<font style="color:#f03d3d">development</font>和<font style="color:#f03d3d">production</font>模式，两种模式分别**约定**对应 本地开发打包 和 线上发布。

通常都是使用不同的命令去执行不同打包操作，在前面虽然也进行了分离，但依然使用的一个配置文件，并没有做到彻底分离。
...
webpack社区中就提供了一个<font style="color:#f03d3d">webpack-merge</font>库，这个库是将多个对象根据属性合并为一个。本质类似于**Object.assign**，只是功能稍微强一些吧了。
...
在使用 *webpack.common.js* 文件进行抽离后，在 webpack.dev.js 和 webpack.pro.js 文件中只具有一个属性，看起来是没有什么必要进行抽离，这是因为目前使用的属性配置基本都是通用的， 当然也有其各自的属性配置，例如 **devServer** ，这个在下面再说。
